 ...
 *************************************************************************/
// Qui vengono definiti i dati statici (la sidebar) – questi dati rimangono invariati
const data = {
  STOCKS: {
    US: [
...
// Helper function per il parsing del gap
function parseGap(val) {
  return (val === "-" || isNaN(parseFloat(val))) ? 0 : parseFloat(val);
}

/*************************************************************************
 * IMPLEMENTAZIONE DELLA FUNZIONE updateSymbolOverview
 *************************************************************************/
// In questa versione, updateSymbolOverview aggiorna un elemento (con id "symbol-overview")
// Mostrando una stringa con il ticker e (eventualmente) alcuni altri dettagli.
// In una versione definitiva potrai personalizzarla ulteriormente.
function updateSymbolOverview(instrumentName) {
  const overviewEl = document.getElementById("symbol-overview");
  if (overviewEl) {
    // Per esempio, impostiamo il testo con alcuni dati di base.
    let details = "";
    if (stocksFullData[instrumentName]) {
      const info = stocksFullData[instrumentName];
      details = `Ticker: ${instrumentName}\nScore: ${info.summaryLeft[0]}\nTrend: ${info.summaryLeft[1]}\nApproach: ${info.summaryLeft[2]}`;
    } else if (etfFullData[instrumentName]) {
      const info = etfFullData[instrumentName];
      details = `Ticker: ${instrumentName}\nScore: ${info.summaryLeft[0]}\nTrend: ${info.summaryLeft[1]}\nApproach: ${info.summaryLeft[2]}`;
    } else if (futuresFullData[instrumentName]) {
      const info = futuresFullData[instrumentName];
      details = `Ticker: ${instrumentName}\nScore: ${info.summaryLeft[0]}\nTrend: ${info.summaryLeft[1]}\nApproach: ${info.summaryLeft[2]}`;
    } else if (fxFullData[instrumentName]) {
      const info = fxFullData[instrumentName];
      details = `Ticker: ${instrumentName}\nScore: ${info.summaryLeft[0]}\nTrend: ${info.summaryLeft[1]}\nApproach: ${info.summaryLeft[3]}`;
    } else {
      details = `Ticker: ${instrumentName}\nDettagli non disponibili.`;
    }
    overviewEl.textContent = details;
  } else {
    console.warn("Elemento con id 'symbol-overview' non trovato nel DOM.");
  }
}
...
function mapStockInstrument(item) {
  return {
    ...
    summaryRight: [
      item.sp500_correlation || 0,
      item.sp500_volatility_ratio || 0,
      item.bullish_alpha || 0,
      item.bearish_alpha || 0,
      item.alpha_strength || 0,
      item.pe_ratio || "",
      item.eps || "",
      item.one_year_high || "",
      item.one_year_low || ""
 ...
 * JSON DATA LOADING
 *************************************************************************/
// Carica il file instruments.json dalla radice del progetto
...
window.addEventListener("DOMContentLoaded", function() {
  loadInstrumentsData().then(() => {
    ...
    // Verifica di quale categoria fa parte lo strumento e aggiorna i relativi blocchi
    if (stocksFullData[currentInstrument]) {
      ...
      updateSymbolOverview(currentInstrument); // Puoi in futuro creare updateSymbolOverviewETF se necessario
 ...
 *************************************************************************/
// [START Portfolio Builder Code]
// Inserisci qui il codice esistente per il portfolio builder, se presente.
...
function renderPortfolio1ChartsFX(portfolioData, barCanvasId, pieCanvasId, distributionFunction) {
  ...
  const distribution = (distributionFunction) ? distributionFunction(portfolioData) : computeFXBaseDistribution(portfolioData);
  const ctxPie = document.getElementById(pieCanvasId).getContext("2d");
  new Chart(ctxPie, {
    type: 'pie',
    data: {
...
function loadThematicPortfolio() {
  ...
  // Esempio per STOCKS
  let portfolio1Data = [];
  Object.keys(stocksFullData).forEach(instrument => {
  ...
  Object.keys(fxFullData).forEach(instrument => {
...
function updateBlock3Generic(instrumentName, dataObj, rowCount, leftLabelArr, rightLabelArr, tradingViewUpdater) {
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (i === 3) {
        const gapVal = info.summaryLeft[i];
...
let currentInstrument = "AMAZON";  // Deve comparire una sola volta
