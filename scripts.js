...
function mapFXInstrument(item) {
  // For FX:
  // summaryLeft: [SCORE, TREND, GAP TO PEAK/TO VALLEY, APPROACH, KEY AREA, LIMIT, POTENTIAL EXTENSION]
  // summaryRight: [AVERAGE DAILY VOLATILITY, FX VOLATILITY RATIO, 30 DAYS PROJECTION, LONG TERM - MACRO, MEDIUM TERM - MATH, MEDIUM TERM - STATS, SHORT TERM - TECH]
  return {
    summaryLeft: [
      item.final_score,      // SCORE
      item.trend,            // TREND
...
// Load instruments.json; modifica il percorso qui in modo da puntare al file nella root
function loadInstrumentsData() {
  return fetch("instruments.json")
    .then(response => response.json())
    .then(dataArray => {
      // dataArray is an array of objects; map them by asset_class
 ...
 * (Le funzioni openFilterSelector, updatePortfolioSteps, generatePortfolioNew, ecc.
 * rimangono sostanzialmente invariate)
 *************************************************************************/
// [START Portfolio Builder Code - invariato]
// Inserisci qui il codice esistente per il portfolio builder (openFilterSelector, updatePortfolioSteps, generatePortfolioNew, etc.)
 ...
 * JSON DATA LOADING
 *************************************************************************/
// Qui carichiamo instruments.json dalla root (instruments.json deve essere in questo percorso)
...
function loadInstrumentsData() {
  return fetch("instruments.json")
    ...
    .then(dataArray => {
      ...
      // Map the instruments into the specific global variables
      dataArray.forEach(item => {
        if (item.asset_class === "stock") {
          stocksFullData[item.ticker] = mapStockInstrument(item);
        } else if (item.asset_class === "etf") {
 ...
 *************************************************************************/
window.addEventListener("DOMContentLoaded", function() {
  // Carica il file JSON e poi inizializza la dashboard
  loadInstrumentsData().then(() => {
    generateSidebarContent();
    currentInstrument = "AMAZON";
    if (stocksFullData[currentInstrument]) {
...
/*************************************************************************
 * PORTFOLIO BUILDER FUNCTIONS
 * (Il codice per openFilterSelector, updatePortfolioSteps, generatePortfolioNew, ecc. rimane invariato)
 *************************************************************************/
// [START Portfolio Builder Code - invariato]
// Inserisci qui il codice esistente per il portfolio builder (openFilterSelector, updatePortfolioSteps, generatePortfolioNew, etc.)
// [END Portfolio Builder Code]
